...
### 1.7 CAD 치수는 어디서 왔나 — 바퀴 D95 기준 최적해

CAD(export 5)의 링크 길이는 **바퀴 지름 95 mm 시절 설정**으로 돌린 asd.py 최적해다. 현재 asd.py 와는 세 파라미터만 다르고, 나머지(질량 5 kg, LF 1.5, $\tau$ 4.5 Nm, 링크 하한 80 mm, $\phi_{MP}$ 135°, 가중치 전부)는 같다.

| 파라미터 | CAD 설계 당시 | 현재 asd.py |
|---|---|---|
| `R` 바퀴 반지름 | **0.0475** (D95) | 0.060 (D120) |
| `H_R_MIN` / `H_R_MAX` | **0.170 / 0.290** | 0.195 / 0.315 |

같은 설정으로 다시 돌리면 CAD 치수가 그대로 나온다 [계산, 2026-09-25 재실행, 25 s]. 바퀴를 120 mm 로 바꾼 뒤에는 링크를 다시 최적화하지 않았다.

| | ① D95 설계 = **CAD** | ② 같은 링크 + 실제 D120 바퀴 | ③ D120 재최적화 (현재 asd.py) |
|---|---|---|---|
| $L_1$ 크랭크 | **102.72 mm** | 102.72 | 107.68 |
| $L_2$ 그라운드 | **100.72** | 100.72 | 98.76 |
| $L_3$ 커플러 | **80.00** (하한) | 80.00 | 80.00 (하한) |
| $L_4$ 생크 | **171.05** | 171.05 | 201.46 |
| $L_5$ 로커 | **80.00** (하한) | 80.00 | 80.00 (하한) |
| 모터 사용 구간 $\theta_a$–$\theta_b$ | 38.0 – 97.8° | 38.0 – 97.8° | 22.6 – 85.9° |
| 고관절 높이 $h$ | 170.0 – 290.0 mm | **182.5 – 302.5 mm** | 195.0 – 315.0 mm |
| 행정 | 120.0 | 120.0 | 120.0 |
| 직선도 RMS / $x_W$ 폭 | 1.34 / 6.6 mm | 1.34 / 6.6 mm | 0.73 / 2.1 mm |
| 전달각 $\mu$ | 49.0 – 146.7° | 49.0 – 146.7° | 30.0 – 123.2° |
| 최대 토크 (LF 1.5, 5 kg) | 4.35 Nm | 4.35 Nm | 4.38 Nm |
| 모터-바퀴 간극 최소 | 35.5 mm | **23.0 mm** | 35.5 mm |
| 모터 지상고 최소 | 130.5 mm | 142.9 mm | 155.5 mm |

- ① → ②: 링크는 그대로이고 바퀴만 커졌다. 그래서 고관절 높이가 반지름 차이 12.5 mm 만큼 올라가고, 모터-바퀴 간극이 12.5 mm 줄어든다($=h-2R-r_m$ 의 $R$ 이 커짐). 직선도·전달각·토크는 링크만의 성질이라 그대로다.
- ③ 은 D120 에 맞춰 다시 풀면 생크가 30 mm 길어지고 간극 35.5 mm 를 되찾는다. 직선도도 1.8 배 좋아진다.
- $L_4$: 설계값 171.05 mm. export 5 URDF 에서 I→W 평면 거리를 직접 재면 170.98 mm (−0.07 mm) 이다. 시뮬 물리는 URDF 형상을 그대로 쓴다.

**결정할 것**: 현재 CAD(②)로 가도 행정·토크·사점 여유는 요구를 만족한다. 다만 모터-바퀴 간극 23 mm 로 충분한지 CAD 간섭 검토가 필요하다. 다음 판에서 ③ 치수로 바꿀지는 계단 등반 시험 전에 정한다.

...
```math
\pi_\phi:\ \underbrace{o_t\in\mathbb{R}^{25}}_{\text{IMU + 관절 + 명령}}\ \longmapsto\ \underbrace{a_t\in[-1,1]^4}_{\text{다리 2 + 바퀴 2}},\qquad 200\ \text{Hz}
```
...
| `Isaac-WheeledBiped-CAD-Rough-v0` | CAD 4절링크 폐루프 | **25** (+높이 모드, IMU 가속도, 고관절 토크) | **거친 지형 커리큘럼** (자갈길·험지 포함) | 짐벌(몸통 고정), 자동/수동 높이 |

거친 지형 과제의 관측은 두 번 늘었다. v2 는 20 (높이 모드 추가), **v3 는 25** (가속도·토크 추가, §4) 다. 크리틱은 v3 부터 지형 스캔을 따로 본다(§4.1, 학습 전용).
...
링크 길이 [설계, Onshape 변수]: $L_1=|MI|=102.72$, $L_2=|MP|=100.72$, $L_3=|IK|=80.00$, $L_4=|IW|=171.05$, $L_5=|PK|=80.00$ mm, 바퀴 $R=60$ mm. `leg_map.py` 는 export URDF 에서 잰 $L_4=170.98$ mm 를 쓴다(−0.07 mm, 다리 길이 영향 0.07 mm 이하).
...
| 12–13 | $\dot h_i$ | $J(\theta_i)\,\dot\theta_i$ (L, R) | m/s | AK60-6 속도 | **0.05** (v2 까지 0.5) |
| 14–15 | $\omega_{w,i}$ | 바퀴 각속도, $+y$ 규약 (L, R) | rad/s | AK45-10 속도 | 0.5 |
| 16–19 | $a_{t-1}$ | 직전 행동 (다리 L, R, 바퀴 L, R) | – | 제어기 내부 | – |
| **20–22** | $f_b/g$ | IMU 비력 $R_{wb}^\top(a_w-g_w)/9.81$, 가만히 있으면 $(0,0,1)$ | g | **iAHRS 가속도** | 0.03 |
| **23–24** | $\tau_{\text{hip},i}/5$ | 고관절 모터 토크 (L, R), + = 다리를 펴는 쪽 | 5 Nm | **AK60-6 전류 × $K_t$** | 0.04 |

- 평지 과제는 9 번($m$)과 20–24 번이 없는 19 차원이다(9 뒤 인덱스가 하나씩 당겨진다). v2 는 20–24 번이 없는 20 차원이다.
- **20–24 번을 넣은 이유**: v2 까지는 정책이 수직 충격을 느낄 신호가 없었다. 가속도계가 없고, $\dot h$ 에는 신호보다 큰 잡음이 붙어 있었다. 그래서 짐벌을 전혀 배우지 못했다(§12.4, 격리율 ≈ 1.0).
...
- v2 까지 12–13 번 $\dot h$ 에 붙인 잡음 ±0.5 m/s 는 단순화 모델(직선관절 속도)에서 물려받은 값이었다. 실제 $\dot h$(대개 0.1 m/s 이하)보다 커서 정책이 이 두 칸을 사실상 못 썼다 → v3 에서 0.05.

### 4.1 크리틱 관측 (145 차원) — 학습 때만 쓴다

**비대칭 액터-크리틱**: 크리틱(가치 함수)만 시뮬에서만 알 수 있는 정보를 더 본다.

```math
o^{\text{critic}}=\big[\,o\ (\text{잡음 없음, 25}),\ \ v_b\ (\text{몸체 선속도, 3}),\ \ z_{\text{COM}}-z_{\text{ground},j}-0.25\ (\text{지형 스캔 }1.2\times0.8\text{ m, 117 점})\,\big]
```

- **실기에는 지형 스캔이 필요 없다.** 크리틱은 PPO 가 이점 $\hat A_t$ 를 계산할 때만 쓰는 "코치"다. 실기에 올라가는 건 정책(액터)뿐이고, 정책 입력 25 개는 전부 실기 센서로 잴 수 있다. `export_policy.py` 도 액터만 내보낸다.
- 효과: 크리틱이 지형을 알면 가치 추정이 정확해진다. 그러면 새 지형에서 이점이 잡음이 되어 엔트로피만 누적되던 문제(§10.2)가 줄어든다.
...
8 × 8 m 타일 10 행(난이도) × 20 열(종류). 행 $r$ 의 난이도는 $d\in[r/10,(r+1)/10)$ 이다. 학습용 seed 42, 측정용 seed 7(보지 못한 지형).

| 종류 | 비율 | 형상 (난이도 $d$) |
|---|---|---|
| flat | 0.15 | 평지 (평지 성능 유지, 수동 모드는 여기서만) |
| **gravel** 자갈길 | 0.15 | 10 cm 격자 무작위 높이 $\pm(0.4+1.6d)$ cm, 평활 없음 |
| **rugged** 험지 | 0.15 | 2 m 기복(피크-피크 $0.06d$ m) + 40 cm 격자 요철 $\pm(0.5+2.5d)$ cm + 10 cm 자갈 $\pm(0.2+1.0d)$ cm 를 겹침 |
| rough | 0.12 | 25 cm 격자 무작위 높이 $\pm(0.5+3.0d)$ cm, 스플라인 보간 |
| wave_long | 0.08 | $z=\tfrac{A}{2}\big(\cos\tfrac{2\pi y}{\lambda}+\sin\tfrac{2\pi x}{\lambda}\big)$, $\lambda=2$ m, $A=0.10d$ |
| wave_short | 0.09 | 같은 식, $\lambda=1$ m, $A=0.06d$ |
| slope_up / down | 0.07 / 0.07 | 피라미드 경사 $\tan\alpha=0.30d$ (최대 16.7°) |
| bumps | 0.12 | 턱·파인 곳 $1+3d$ cm, 폭 0.3–1.0 m, 10 cm 에 걸친 경사로 |
...
### 10.3 관측 v2 → v3 가중치 변환 (`scripts/add_obs_v3.py`)

새 입력(정책 5, 크리틱 125)을 첫 층 **뒤에** 0 열로 붙인다 → 처음 동작은 v2 와 같다.

정규화기의 새 칸은 사전값(가속도 $z$ 평균 1 g 등)으로 채운다. 누적 개수 count 는 $10^6$ 으로 낮춘다. 예전 count(수억)를 그대로 두면 새 칸 통계가 거의 움직이지 않는다. Adam 모멘트도 같은 모양으로 늘려 보존한다.

### 10.4 이어서 학습 vs 처음부터 — r4 부터는 처음부터

지금까지(r2, r3)는 앞 정책에서 **이어서** 학습했다. 빠르지만 다음 문제가 쌓였다.

- **옵티마이저 상태까지 물려받는다.** r3 는 엔트로피 0.001 로 std 가 0.19 까지 줄었다. 작은 std 에서는 같은 평균 변화에도 KL 이 커서 adaptive 스케줄이 학습률을 **하한 1e-5** 까지 깎았다. r3 에서 이어 받은 r4 시험도 시작부터 1e-5 라 거의 배우지 못했다(`pv health` 가 잡음).
- **버전 비교가 안 된다.** 좋아진 게 설정 덕인지 누적 학습량 덕인지 구분할 수 없다.
- **수술(관측·행동 차원 변경 3 회)과 옛 버릇이 누적된다.** 재현하기 어렵다.

그래서 r4 부터는 오리 로봇처럼 **버전 = 설정 하나, 처음부터** 학습한다(엔트로피 0.006). 이어서 학습은 짧은 시험에만 쓴다(`pv train <ver> --from`).

**수렴 자동 정지** (`scripts/train_health.py`, 오리 리포에서 가져와 고침, `pv autostop`): 텐서보드 이벤트만 읽는다(Isaac 안 띄움). 5 분마다 판정하고, 아래가 모두 참이면 학습을 멈춘다.

```math
\Big|\tfrac{\Delta\,\text{지형 단계}}{\text{지형 단계}}\Big|<3\,\%,\quad |\Delta\,\text{완주 env}|<2\,\%\text{p},\quad \Big|\tfrac{\Delta\,\text{초당 추종}}{\text{초당 추종}}\Big|<2\,\%\quad(\text{최근 400 iter, 양 끝 50 iter 평균}),\qquad \text{완주 env}\ge0.5
```

- 초당 추종 = (track_lin + track_ang) / (alive / 2) 다. `Episode_Reward/*` 는 짧은 에피소드 쪽으로 치우쳐서 생존 시간으로 나눈다.
- 완주 ≥ 0.5 조건은 "아무것도 못 배워서 평평한" 초반을 수렴으로 오판하지 않기 위해서다.
- 처음부터 학습이면 최소 1500 iter, 이어서면 500 iter 뒤부터 판정한다.
- 경고: 학습률 바닥, std 붕괴(< 0.08)·과대(> 0.75), 지형 단계 퇴행(−10 %).
...
| 이어 학습 r3 끝에서 학습률 1e-5 (하한) | 엔트로피 0.001 → std 0.19 → 작은 std 에서 KL 과대 → adaptive 스케줄이 lr 을 바닥까지 | r4 부터 처음부터 학습, $c_e$ 0.006 (§10.4) |
...
### 12.5 관측 v3 + 비대칭 크리틱 + 자갈길·험지 (model_5600 → 7099) [측정]

보지 못한 지형(seed 7) 9 종 × 4 난이도 × 3 개체, 전진 0.5 m/s. 5600 = v3 로 옮긴 직후(동작은 v2 와 같음).

| | 5600 | 6400 | **7099** |
|---|---|---|---|
| 자동: 낙상 | 0/108 | 0/108 | **0/108** |
| 자동: roll RMS | 1.32° | 0.42° | **0.42°** |
| 자동: 정지 중 밀림 | 30.4 cm | 5.4 | **4.4 cm** |
| 자동: 속도 (명령 0.5) | 0.32 m/s | 0.48 | **0.50** |
| 자동: 몸통 수직가속 RMS | 1.11 m/s² | 0.65 | **0.66** |
| 자동: 다리 평균 $h$ | 130 mm | 141 | **144** |
| 자동: 격리율 (참고) | 0.99 | 1.09 | 1.03 |
| 수동(평지): 수직가속 / roll | 0.89 / 1.08° | 0.38 / 0.39° | **0.26 / 0.37°** |
| 수동(평지): 정지 중 밀림 | 1.7 cm | 14.9 | **10.4 cm** ✗ |

- 자갈길·험지를 포함한 전 지형에서 낙상이 없다.
- 좌우 수평(3 배), 정지 유지(7 배), 속도 추종이 크게 좋아졌다. 관측에 넣은 IMU 가속도·고관절 토크와 지형을 보는 크리틱의 효과로 본다.
- **남은 문제**: 수동 모드 평지 정지 밀림 퇴행(1.7 → 10 cm).

### 12.6 "짐벌" 의 정의를 바로잡음 — 고정이 아니라 **부드럽게**

사용자 정의: 몸통이 한 높이에 **고정될 필요는 없다.** 오르내리더라도 **부드럽게** 움직이면 된다(카메라 짐벌·스테디캠).
그래서 격리율(높이 **변위** 비율)은 주 지표에서 내린다. 몸통이 긴 기복을 천천히 따라가면 격리율이 1 이어도 목표를 달성한 것이다.

부드러움은 **가속도**로 잰다.

```math
\text{가속도 전달률}\ T_a=\frac{\operatorname{RMS}(a_{z,\text{몸통}})}{\operatorname{RMS}(a_{z,\text{바퀴축}})}\qquad(T_a<1:\ \text{다리가 충격을 걸러 줌})
```

보조 지표: 몸통 수직가속 RMS(승차감, ISO 2631 축), roll rate RMS. 학습 보상도 이미 같은 방향이다. `ride` 는 몸통 수직가속도에, `gimbal_height` 는 넓은 지면 평균(공간 저역통과)에 걸려 있다. 전달률 측정 결과는 다음 측정에서 채운다.

---

## 13. 실기 이식 체크리스트

1. 관측 25 칸의 **순서·단위·부호**를 §4 표대로 맞춘다. 특히 $g_b$ 부호, 바퀴 $+y$ 규약, IMU 비력(단위 g, 가만히 있으면 $z=+1$), 고관절 토크 부호(+ = 다리 펴는 쪽). 지형 스캔은 필요 없다(크리틱 전용).

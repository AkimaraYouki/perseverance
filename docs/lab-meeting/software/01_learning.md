...
- **높이 모드 $m$** (Rough 과제만, 평지 과제에는 없어서 명령이 3 개):
  - $m=0$ **수동**: 평균 높이 = 사용자가 준 $h_{\text{ref}}$. 좌우 수평과 요철 흡수는 계속 한다.
  - $m=1$ **자동**: 평균 높이까지 정책이 외란·지형에 맞춰 정한다. $h_{\text{ref}}$ 는 공칭값 $h_0=136.5$ mm(CAD 기본자세 $M=0$, IDLE)로 고정되고, 보상에서 느슨한 선호로만 쓴다(§5.2).
...
&m\sim\text{Bernoulli}(0.5),\quad m=1\Rightarrow h_{\text{tgt}}=h_0
\end{aligned}
```
...
```math
f_{\text{ride}}=\exp\!\Big(-\frac{a_{z}^2}{3^2}\Big)\quad[\text{m/s}^2]
```
...
| 엔트로피 $c_e$ | 0.006 (0.005: std 붕괴 0.23 → 탐색 사망, 0.02: std 1.87 로 폭주) |
...
### 12.3 거친 지형 + 높이 모드 학습 후

학습 진행 중이다(model_4700 → 6200). 끝나면 같은 측정으로 이 표를 채운다.
